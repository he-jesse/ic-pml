# Author: Sharon Zhang
# https://github.com/sxzhang25/product-manifold-learning
...
# Modified: Jesse He
def calc_W(data, sigma, self_tuning = False, k_neighbors = 7, threshold = False):
  '''
  ...
  if self_tuning:
    loc_scale = np.partition(pairwise_sq_dists, k_neighbors, axis=1)[:,k_neighbors]
    scale_mat = sigma * np.outer(loc_scale,loc_scale)
    W = np.exp(-pairwise_sq_dists / scale_mat)
    if threshold:
      W[pairwise_sq_dists > scale_mat] = 0
  else:
    W = np.exp(-pairwise_sq_dists / sigma)
    if threshold:
      W[pairwise_sq_dists > 4*sigma] = 0
  return W
...
def calculate_sim(v_i, v_j):
  ...
  sim = np.abs(np.dot(v_i, v_j))
  return sim # score

def find_combos(phi, Sigma, n_factors=2, eig_crit=10e-3, sim_crit=0.5, exclude_eigs=None, ind_eigs = None):
  '''
  ...
  for k in range(2, phi.shape[1]): # k is the proposed product eigenvector
    ...
    for m in range(2, n_factors + 1):
      ...
      # ---
      elif ind_eigs is not None:
        valid_eigs = [v for v in np.arange(1, k) if v in ind_eigs]
      # ---
      else:
